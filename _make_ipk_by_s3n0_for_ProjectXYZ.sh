...
cat > ${project_dir}/CONTROL/preinst << EOF
#!/bin/sh
rm -rf ${plugin_dir}
exit 0
EOF
...
cat > ${project_dir}/CONTROL/prerm << EOF
#!/bin/sh
exit 0
EOF
...
#### using the "ar" archiver to merge all 3 files in the following order:   1) debian-binary   2) data.tar.gz   3) control.tar.gz
cd ${build_dir}
rm -f ${ipk_targetfile}
ar -r ${ipk_targetfile} ./debian-binary ./data.tar.gz ./control.tar.gz 
